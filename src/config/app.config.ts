...
class DbConfig {
	static readonly MONGODB_URI =
		process.env.MONGODB_URI || 'mongodb+srv://venkatesh:venkatesh@amplify-test.1eigwf3.mongodb.net/?retryWrites=true&w=majority';
...
class AuthConfig {
	...
	static readonly PASSWORD_PATTERN = process.env.PASSWORD_PATTERN || /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/;
}
